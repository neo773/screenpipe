...
export default function Permissions({ handleNextSlide, handlePrevSlide, className }: PermissionsProps) {
  ...
  const handlePermissionRequest = async (type: 'screen' | 'audio') => {
    try {
      const permissionType = type === 'screen' ? 'screenRecording' : 'microphone'
  ...
  const isAllGranted = permissions.screenRecording === 'Granted' && 
  ...
  return (
    ...
    >
        ...
        <Card className="flex justify-between items-start p-6" >
          ...
          <Button 
            variant={permissions.screenRecording === 'Granted' ? 'secondary' : 'default'}
            className="w-32"
            onClick={() => handlePermissionRequest('screen')}
            disabled={permissions.screenRecording === 'Granted'}
          >
            {permissions.screenRecording === 'Granted' ? 'granted' : 'grant access'}
          </Button>
        </Card>
        ...
        <Card className="flex justify-between items-start p-6">
          ...
          <Button 
            variant={permissions.microphone === 'Granted' ? 'secondary' : 'default'}
            className="w-32"
