...
await client.add.execute({ content: { content_type: 'content_type', data: [{ app_name: 'app_name', file_path: 'file_path', ocr_results: [{ focused: true, ocr_engine: 'ocr_engine', text: 'text', text_json: 'text_json' }], tags: ['string'], timestamp: '2019-12-27T18:11:19.117Z', window_name: 'window_name' }] }, device_name: 'device_name' }, {
  maxRetries: 5,
});
```
...
const { data: response, response: raw } = await client.add
  .execute({
    content: {
      content_type: 'content_type',
      data: [
